...
if __name__ == "__main__":
    ...
    resp = requests.post("http://localhost:8000/weather/api/", json=data)
    print("POST: %s: %s" % (resp.status_code, str(resp.json())))
    if resp.status_code == 202:

        progress = 0.0
        while progress < 100.0:
            resp = requests.get("http://localhost:8000/weather/api/?user_id=%s" % data['user_id'])
            answer = resp.json()
            print("GET: %s: %s" % (resp.status_code, str(answer)))
